...
const Heading: React.FC = () => (
  <>
    ...
    <p className="text-2xl font-thin mt-4 uppercase text-center text-white/80">
      India’s biggest literature festival
    </p>
  </>
);

// Image Carousel Component
const ImageCarousel: React.FC<{ images: string[]; edition: string }> = ({
  images,
  edition,
}) => {
...
const EventsJourney: React.FC = () => {
  ...
  const getCardPosition = (eventId: number): { x: number; y: number } => {
    ...
    let cardY = nodePosition.isAbove
      ? nodePosition.y - 10
      : nodePosition.y + 10;
